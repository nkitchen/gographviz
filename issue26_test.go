...
func TestIssue26DefaultAttrs(t *testing.T) {
	...
	st, _ := ParseString(inputString)

	var userStmt ast.NodeStmt
	var depStmt ast.NodeStmt
	for _, stmt := range st.StmtList {
		if ns, ok := stmt.(ast.NodeStmt); ok {
			switch {
				case ns.NodeID.ID == "user":
					userStmt = ns
					case ns.NodeID.ID == "dep":
					depStmt = ns
			}
		}
	}
	fmt.Printf("user attrs %v\n", userStmt.Attrs)
	fmt.Printf("dep  attrs %v\n", depStmt.Attrs)
	...
	for _, c := range []nodeCase{
		...
		if value := node.Attrs[c.attr]; value != c.value {
			t.Errorf("expected %s=%#v in %s; got %#v", c.attr, c.value, c.name, value)
		}
	}
	...
	for _, c := range []edgeCase{
		...
		for _, e := range edges {
			if value := e.Attrs[c.attr]; value != c.value {
				t.Errorf("expected %s=%#v in %s->%s; got %#v", c.attr, c.value,
					c.src, c.dst, value)
			}
		}
	}
